 ...
 * スプレッドシートから入力データを取得
 */
function getInputData(spreadsheet: GoogleAppsScript.Spreadsheet.Spreadsheet): InputData {
  ...
  const data: InputData = {
    documentType: inputSheet.getRange(CONFIG.CELLS.DOCUMENT_TYPE).getValue() as DocumentType,
    issueDate: inputSheet.getRange(CONFIG.CELLS.ISSUE_DATE).getValue() as Date,
    companyName: inputSheet.getRange(CONFIG.CELLS.COMPANY_NAME).getValue() as string,
    contactName: inputSheet.getRange(CONFIG.CELLS.CONTACT_NAME).getValue() as string,
    address: inputSheet.getRange(CONFIG.CELLS.ADDRESS).getValue() as string,
    email: inputSheet.getRange(CONFIG.CELLS.EMAIL).getValue() as string,
    remarks: inputSheet.getRange(CONFIG.CELLS.REMARKS).getValue() as string,
    items: getItemsData(inputSheet),
    totalAmount: inputSheet.getRange(CONFIG.CELLS.TOTAL_AMOUNT).getValue() as number,
    tax: inputSheet.getRange(CONFIG.CELLS.TAX).getValue() as number,
    grandTotal: inputSheet.getRange(CONFIG.CELLS.GRAND_TOTAL).getValue() as number
  };
 ...
 * 商品明細データを取得
 */
function getItemsData(inputSheet: GoogleAppsScript.Spreadsheet.Sheet): ItemData[] {
 ...
 * 入力データの検証
 */
function validateInputData(data: InputData): boolean {
  const errors: string[] = [];
 ...
 * PDFを生成
 */
function generatePDF(spreadsheet: GoogleAppsScript.Spreadsheet.Spreadsheet, inputData: InputData): GoogleAppsScript.Base.Blob {
  // テンプレートシートを取得
 ...
 * テンプレートシートにデータを反映
 */
function updateTemplateSheet(templateSheet: GoogleAppsScript.Spreadsheet.Sheet, inputData: InputData): void {
  // 基本情報を設定
 ...
 * テンプレートシートの商品明細を更新
 */
function updateItemsInTemplate(templateSheet: GoogleAppsScript.Spreadsheet.Sheet, items: ItemData[]): void {
 ...
 * PDFファイル名を生成
 */
function generateFileName(inputData: InputData): string {
 ...
 * PDFをフォルダに保存
 */
function savePDFToFolder(pdfBlob: GoogleAppsScript.Base.Blob, inputData: InputData): GoogleAppsScript.Drive.File {
 ...
 * メール送信
 */
function sendEmailWithPDF(pdfBlob: GoogleAppsScript.Base.Blob, inputData: InputData): void {
 ...
 * 送信履歴を記録
 */
function recordSendingHistory(spreadsheet: GoogleAppsScript.Spreadsheet.Spreadsheet, inputData: InputData, savedFile: GoogleAppsScript.Drive.File): void {
 ...
 * バックアップドキュメントを作成
 */
function createBackupDocument(inputData: InputData, savedFile: GoogleAppsScript.Drive.File): void {
  const parentFolder = DriveApp.getFileById(SpreadsheetApp.getActiveSpreadsheet().getId()).getParents().next();
