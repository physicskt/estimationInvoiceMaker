 ...
 * @return フォルダオブジェクト
 */
function getOrCreateFolder(parentFolder: GoogleAppsScript.Drive.Folder, folderName: string): GoogleAppsScript.Drive.Folder {
 ...
 * @return 送信履歴シート
 */
function getOrCreateHistorySheet(spreadsheet: GoogleAppsScript.Spreadsheet.Spreadsheet): GoogleAppsScript.Spreadsheet.Sheet {
 ...
 * スプレッドシートに入力シートを作成
 */
function createInputSheet(): GoogleAppsScript.Spreadsheet.Sheet {
 ...
 * @param sheet 入力シート
 */
function setupInputSheetLayout(sheet: GoogleAppsScript.Spreadsheet.Sheet): void {
  // ヘッダー
  ...
  labels.forEach(([label, , note], index) => {
    const row = index + 2;
 ...
 */
function createTemplateSheet(): GoogleAppsScript.Spreadsheet.Sheet {
  const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
 ...
 */
function setupTemplateSheetLayout(sheet: GoogleAppsScript.Spreadsheet.Sheet): void {
  // 書類タイトル
...
function initialSetup(): void {
  try {
    ...
    SpreadsheetApp.getUi().alert(`初期セットアップでエラーが発生しました: ${errorMessage}`);
  }
}
  ...
  try {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      if (row[0] && row[1] && row[2]) { // 品目、数量、単価が入力されている場合
        ...
        const unitPrice = parseFloat(String(row[2])) || 0;
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    SpreadsheetApp.getUi().alert('エラー', `計算中にエラーが発生しました: ${errorMessage}`, SpreadsheetApp.getUi().ButtonSet.OK);
  }
}
...
function clearInputData(): void {
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    SpreadsheetApp.getUi().alert('エラー', `クリア中にエラーが発生しました: ${errorMessage}`, SpreadsheetApp.getUi().ButtonSet.OK);
  }
}
...
function checkSystemStatus(): void {
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    SpreadsheetApp.getUi().alert('エラー', `システム確認中にエラーが発生しました: ${errorMessage}`, SpreadsheetApp.getUi().ButtonSet.OK);
  }
}